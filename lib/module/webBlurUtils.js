 ...
 */
export const BLUR_TYPE_TO_BACKGROUND = {
  // Base types
...
const cssNumber = value => Number(value.toFixed(2));
let backdropFilterSupport;
...
export function supportsBackdropFilter() {
  if (backdropFilterSupport === undefined) {
    ...
    backdropFilterSupport = css.supports('backdrop-filter', 'blur(1px)') || css.supports('-webkit-backdrop-filter', 'blur(1px)');
  }
...
export function useBackdropFilterSupport() {
  return useSyncExternalStore(subscribeToBackdropSupport, supportsBackdropFilter, serverBackdropSupport);
}
 ...
 */
export function getBlurLayerStyle(blurType, blurAmount, hasBackdropFilter = supportsBackdropFilter()) {
  const style = {
    backgroundColor: BLUR_TYPE_TO_BACKGROUND[blurType] ?? BLUR_TYPE_TO_BACKGROUND.regular
  };
...
const pct = value => `${cssNumber(value)}%`;
const maskForLayer = (direction, startOffset, layer) => {
  ...
  if (direction === 'blurredCenterClearTopAndBottom') {
    ...
    const q = j => 30 - inset * 100 + 20 * j / n;
    const inner = q(n - layer);
    const outer = q(n - layer + 1);
    return `linear-gradient(to bottom, transparent ${pct(50 - outer)}, ` + `black ${pct(50 - inner)}, black ${pct(50 + inner)}, ` + `transparent ${pct(50 + outer)})`;
  }

  // Band edges measured from the blurred edge (0%).
  const boundary = j => startOffset * 100 + (1 - startOffset) * 100 * j / n;
 ...
 */
export function getProgressiveLayers(direction, startOffset, blurAmount) {
 ...
 */
export function getProgressiveTintGradient(direction, startOffset, tint) {
  const s = clamp(startOffset, 0, 1);
  if (direction === 'blurredCenterClearTopAndBottom') {
    ...
    return `linear-gradient(to bottom, transparent ${pct(inset)}, ${tint} ${pct(inset + 20)}, ` + `${tint} ${pct(80 - inset)}, transparent ${pct(100 - inset)})`;
  }
  const axis = direction === 'blurredBottomClearTop' ? 'to top' : 'to bottom';
  return `linear-gradient(${axis}, ${tint} 0%, ${tint} ${pct(s * 100)}, transparent 100%)`;
}
//# sourceMappingURL=webBlurUtils.js.map