 ...
 */
export function getFallbackOverlayColor(tint, opacity) {
  if (!tint) return undefined;
  const normalized = tint.trim().toLowerCase();
  if (normalized === '' || normalized === 'clear' || normalized === 'transparent') {
    return undefined;
  }
  ...
  if (hexBody) {
    if (hexBody.length === 3 || hexBody.length === 4) {
      ...
      if (hexBody.length === 8) tintAlpha = parseInt(hexBody.slice(6), 16) / 255;
    }
  }
  if (rgb) {
    const clamped = Math.max(0, Math.min(1, opacity));
    const alpha = Math.round(clamped * MAX_FALLBACK_TINT_ALPHA * tintAlpha * 255).toString(16).padStart(2, '0');
    return `#${rgb}${alpha}`;
  }
